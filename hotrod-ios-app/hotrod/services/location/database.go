...
// getSeedData returns different location sets based on database environment
func getSeedData() []Location {
	// Base locations for all environments
	...
	// Add enhanced locations only for sandbox database
	dbName := os.Getenv("MYSQL_DATABASE")
	if dbName == "hotrod_sandbox" {
		enhancedLocations := []Location{
			{
				ID:          777,
				Name:        "Egyptian Treats",
				Coordinates: "878,576",
			},
		}
		baseLocations = append(baseLocations, enhancedLocations...)
	}
...
func newDatabase(logger log.Factory) *database {
	...
	targetDB := os.Getenv("MYSQL_DATABASE")
	if targetDB == "" {
		targetDB = config.GetMySQLDatabaseName()
	}
	...
	baseConfig := driverConfig()
	baseConfig.DBName = "" // Connect without specific database
	for {
		db, err = sqlx.ConnectContext(context.TODO(), "mysql", baseConfig.FormatDSN())
		if err == nil {
			break
		}
	...
	_, err = db.Exec(fmt.Sprintf("CREATE DATABASE IF NOT EXISTS `%s`", targetDB))
	if err != nil {
		logger.Bg().Error("error creating database", zap.Error(err), zap.String("database", targetDB))
	} else {
		logger.Bg().Info("ensured database exists", zap.String("database", targetDB))
	}
	db.Close()
	...
	for {
		...
		logger.Bg().Error("error connecting to target db", zap.Error(err))
		<-ticker.C
	}
...
func driverConfig() *mysql.Config {
	...
	// Allow sandbox to use different database name via environment variable
	dbName := os.Getenv("MYSQL_DATABASE")
	if dbName == "" {
		dbName = config.GetMySQLDatabaseName() // Default to baseline
	}
	dc.DBName = dbName
